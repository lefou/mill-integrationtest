...
This plugin provides a `CrossMillIntegrationTestModule` template, which can be used with `Cross` to test plugins across
different versions of Scala.
[source,scala]
----
object itest extends Cross[IntegrationTestModule]("2.13.2", "2.12.11")
class IntegrationTestModule(val crossScalaVersion: String) extends CrossMillIntegrationTestModule {
  def millTestVersion  = if (crossScalaVersion.startsWith("2.13")) "0.6.2-35-7d1144" else "{exampleMillVersion}"
  def pluginsUnderTest = Seq(demo(crossScalaVersion))
}
----
